...
## 아키텍처 구성 (4파트)
1. **AI 에이전트 파트** — `src/agent.py`, `src/tools.py`, `src/retriever.py` (아래 폴더 구조의 제출 규약 3종)
2. **모델 서빙 FastAPI 서버 파트** — `server.py`, `POST /query` 하나만 제공
3. **UI 파트** — `static/chat.html` 하나. 빌드 없이 CDN include 로 채팅 UI 구성
4. **크롤링 & RAG 구축 파트** — `crawl_confluence.py` (data/urls.txt 를 읽어 data/raw/ 에 수집, 첨부 이미지는 저비용 비전 모델로 분류·텍스트화해 본문에 삽입) + `src/retriever.py` 의 인덱싱 함수 (data/raw/ → chroma_db 임베딩)

## 폴더 구조
```
crawl_confluence.py   컨플루언스 문서 수집 (data/urls.txt → data/raw/)
server.py             FastAPI 서버, POST /query
static/chat.html      채팅 UI (단일 HTML, 라이브러리는 CDN include)
src/agent.py          메인 에이전트 그래프        ┐
src/tools.py          도메인 도구 (MCP 서버)      ├─ 제출 규약. 이 3개 파일 역할은 바꾸지 않는다
src/retriever.py      RAG 파이프라인              ┘
src/models.py         Bedrock 모델 폴백 후보 목록 (agent/retriever 공용)
